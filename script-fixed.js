...
function renderStages() {
    ...
    stages.forEach(stage => {
        ...
        if (!isUnlocked) {
            ...
            // عرض الكلمة السرية للمراحل المكتملة (ما عدا المرحلة 8)
            if (stage.secretWord && stage.id !== 8) {
                statusText += `<br><small style="color: var(--accent-yellow);">🔑 الكلمة السرية: ${stage.secretWord}</small>`;
            }
        } else {
...
function renderChallenges(stage, stageProgress) {
    ...
    // ✅ التحقق من تحميل دوال المرحلة 8
    if (stage.id === 8) {
        if (typeof window.createSecretWordPuzzle === 'undefined' || 
            typeof window.createCodeEditor === 'undefined' || 
            typeof window.MarioGame === 'undefined') {
            container.innerHTML = `
                <div style="background: rgba(231, 76, 60, 0.2); padding: 2rem; border-radius: 10px; text-align: center; border: 2px solid var(--danger-red);">
                    <h3>⚠️ خطأ في تحميل المرحلة</h3>
                    <p>لم يتم تحميل ملفات المرحلة الثامنة بشكل صحيح</p>
                    <p style="color: var(--text-gray); margin: 1rem 0;">تأكد من وجود ملف <code>stage8-interactive.js</code> وتحميله بشكل صحيح</p>
                    <button onclick="location.reload()" class="btn-primary" style="margin-top: 1rem;">
                        🔄 إعادة تحميل الصفحة
                    </button>
                </div>
            `;
            console.error('Stage 8 interactive functions not loaded!');
            console.error('Missing functions:', {
                createSecretWordPuzzle: typeof window.createSecretWordPuzzle,
                createCodeEditor: typeof window.createCodeEditor,
                MarioGame: typeof window.MarioGame
            });
            return;
        }
    }
    ...
    stage.challenges.forEach((challenge, index) => {
        ...
        // تحديد نوع التحدي والكلاس المناسب
        if (challenge.type === 'secretWord') {
            challengeDiv.className = 'secret-word-section';
        } else if (challenge.type === 'codeGame') {
            challengeDiv.className = 'code-game-section';
        } else if (challenge.type === 'quiz') {
            challengeDiv.className = 'quiz-section';
        } else {
            challengeDiv.className = 'puzzle-section';
        }
        ...
        if (!isCompleted) {
            // ✅ المرحلة 8 - التحديات التفاعلية مع معالجة الأخطاء
            if (challenge.type === 'secretWord') {
                try {
                    challengeHTML += window.createSecretWordPuzzle(challenge, index);
                    challengeHTML += `
                    ...
                    console.error('Error creating secret word puzzle:', error);
                    challengeHTML += `
                        <div style="background: rgba(231, 76, 60, 0.2); padding: 1rem; border-radius: 10px; color: var(--danger-red);">
                            ❌ خطأ في تحميل لغز الكلمات السرية
                        </div>
                    `;
                }
            } else if (challenge.type === 'codeGame') {
                try {
                    challengeHTML += window.createCodeEditor(challenge, index);
                    challengeHTML += `
                        <div style="margin-top: 2rem; text-align: center;">
                            <button class="submit-answer" data-challenge="${index}">
                                ✅ أكملت اللعبة - احصل على النقاط
                            </button>
                        </div>
                        <div id="feedback-${index}" class="feedback-message" style="display: none;"></div>
                    `;
                    ...
                    challengeHTML += `
                        <div style="background: rgba(231, 76, 60, 0.2); padding: 1rem; border-radius: 10px; color: var(--danger-red);">
                            ❌ خطأ في تحميل محرر الكود ولعبة Mario
                        </div>
                    `;
                }
            } else if (challenge.type === 'quiz') {
                challengeHTML += `<p style="font-size: 1.1rem; margin: 1rem 0;">${challenge.question}</p>`;
        ...
        // ✅ تهيئة لعبة Mario بشكل آمن
        if (challenge.type === 'codeGame' && !isCompleted) {
            setTimeout(() => {
                try {
                    const canvasId = `marioCanvas-${index}`;
                    ...
                    if (typeof window.MarioGame === 'undefined') {
                        console.error('MarioGame class not defined');
                        return;
                    }
                    
                    window.marioGame = new window.MarioGame(canvasId);
                    window.marioGame.draw(); // رسم الحالة الأولية
...
async function checkAnswer(stage, challengeIndex) {
    ...
    try {
        // التحقق حسب نوع التحدي
        if (challenge.type === 'secretWord') {
            if (typeof window.checkSecretWord === 'undefined') {
                throw new Error('checkSecretWord function not loaded');
            }
            isCorrect = window.checkSecretWord(challengeIndex);
        } else if (challenge.type === 'codeGame') {
            if (typeof window.checkMarioGameCompletion === 'undefined') {
                throw new Error('checkMarioGameCompletion function not loaded');
            }
            // التحقق من أن اللعبة تم إكمالها
            isCorrect = window.checkMarioGameCompletion();
            if (!isCorrect) {
                feedbackDiv.className = 'feedback-message error';
                feedbackDiv.textContent = '✗ يجب أن تجمع 50 نقطة في اللعبة أولاً!';
            ...
            // التحقق من اللغز الذكي للمرحلة 8
            if (challenge.alternativeAnswers) {
                if (typeof window.checkSmartPuzzle === 'undefined') {
                    throw new Error('checkSmartPuzzle function not loaded');
                }
                isCorrect = window.checkSmartPuzzle(challengeIndex);
            } else {
        ...
        feedbackDiv.className = 'feedback-message error';
        feedbackDiv.textContent = '✗ حدث خطأ أثناء التحقق من الإجابة. الرجاء إعادة تحميل الصفحة.';
        feedbackDiv.style.display = 'block';
        return;
    }
...
function showStageCompletionMessage(stage, score) {
    ...
    // عرض الكلمة السرية للمراحل (ما عدا المرحلة 8)
    if (stage.secretWord && stage.id !== 8) {
        message += `🔑 الكلمة السرية لهذه المرحلة: ${stage.secretWord}\n`;
        message += `(احفظها - ستحتاجها في المرحلة الأخيرة!)\n\n`;
    }
